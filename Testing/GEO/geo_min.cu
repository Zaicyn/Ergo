// geo_32D_1024_streams_fixed.cu
// 32D Mixed Sizes @ 1024 Concurrent Streams - FIXED, SAFE & BREATHING
...
// === Precomputed trig tables (added to fix undefined identifier) ===
#define TRIG_TABLE_SIZE 4096
#define TRIG_TABLE_STEP (2.0f * (float)M_PI / (float)TRIG_TABLE_SIZE)
...
inline float fast_sinf(float x) {
    float scaled = fmodf(x, 2.0f * (float)M_PI) / TRIG_TABLE_STEP;
    int idx = (int)scaled;
    if (idx < 0) idx += TRIG_TABLE_SIZE;
...
double run_32D(bool use_protection, const char* label) {
    ...
    for (int cycle = 0; cycle < CYCLES; ++cycle) {
        for (int s = 0; s < NUM_STREAMS; ++s) {
            int bin_idx = (cycle % 4);  // period-4 binning
            size_t size = bin_sizes[bin_idx];

            void* ptr = nullptr;

            // Memoization: exact size match, LRU pop
            if (hold_counts[s] > 0) {
...
int main() {
    ...
    printf("Kaze-style precomputed trig tables - reality engine mode\n\n");

    // Initialize trig tables ONCE - then pure lookups
    init_trig_tables();
