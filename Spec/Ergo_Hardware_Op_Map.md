...
### Contraction policy (2026-08-29 — compiler-owned, plan A landed)

**Who may contract a `a*b ± c` into an FMA is now decided by the
Ergo compiler — identically on both paths — not by gcc's discretion
and not by the Vulkan driver's:**

- **The analysis** (`core/ir_contract.py:compute_fma_sites`) walks the
  final IR in program order and classifies every REAL `a*b ± c` site:
  - *deterministic* — both multiply factors are plain values. Both
    backends emit an explicit fused multiply-add at exactly these
    sites: C `fma()`/`fmaf()` on the CPU, `OpExtInst GLSL.std.450 Fma`
    on the GPU. Same site, same instruction, both sides.
  - *fragile* (call-factor) — a multiply factor is the direct result
    of an intrinsic/call (e.g. `S += SIN(X)*0.37`). gcc's fusion at
    these sites depends on post-inlining PHI/sink behavior that is not
    computable from the IR (in one and the same loop the sin site
    fused while the cos site did not; tests/contraction/probe9 flips a
    minimal pair the other way). So the CPU backend emits the
    multiply, a zero-instruction fusion barrier
    (`asm volatile("" : "+x"(v))`) that gcc cannot fuse past, then the
    plain add; the GPU emits the plain ops under `NoContraction`.
    Both sides execute the unfused form.
  - *residual* (GPU only) — a marked site whose product is computed
    host-side (factor ids not resolvable in the kernel) falls back to
    plain ops and is counted in the lint as a residual CPU≠GPU
    boundary.
- **Everything else** the SPIRV backend emits carries a
  `NoContraction` decoration on every floating-point arithmetic
  result — the Vulkan driver may never fuse what the compiler did not
  choose. This was NOT free: the RTX 2060 driver does contract
  undecorated `OpFMul`/`OpFAdd` pairs (measured on
  tests/fusible_stress.ergo: R1(4096) = …97 fused vs …88 unfused).
- **The lint** (`--target spirv`) reports per kernel only the fragile
  and residual sites (deterministic sites are equal by construction,
  silent). A kernel with no reported sites, no transcendentals
  (§9.10), and no reductions (§9.9) is CPU==GPU-bitwise by
  construction — verified: tests/fusible_stress.ergo is CPU==GPU
  byte-identical at f64 AND f32 (it deliberately was not, before).
- **Why compiler-owned instead of matching gcc's fusion set:** gcc's
  fused set is not computable from the IR (the PHI/sink luck above),
  and any uniform rule moves CPU bits — a naive emit-fma-everywhere
  rule moved 145/197 corpus programs. The move was therefore taken
  inside the 2026-08-29 re-certification window: corpus baselines
  re-recorded (`tests/golden/run_corpus.py --record`, 197/197, audit
  green), the certified ul18 CSVs regenerated as byte-reproducible GPU
  f32 double-runs, and the two IR-vs-legacy A2 divergences this
  creates carried as KNOWN (tests/golden/KNOWN_DIVERGENCES.md).
