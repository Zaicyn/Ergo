/* udp_cpeer.c -- C sender/receiver for asm interop. Role via argv[1].
 * Same datagrams, same verdict lines as udp_node.asm:
 *   sender: "S sent=9 served=<n>"
 *   receiver: "V got=<g> lost=<nl> fetch=<f> full=<u>"
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
static uint8_t sbuf[528], rbuf[2048];
static void txmsg(int fd, uint16_t seq, uint8_t kind, uint8_t idx,
                  const uint8_t *pl, uint16_t len,
                  struct sockaddr_in *dst) {
    sbuf[0] = (uint8_t)(seq & 255);
    sbuf[1] = (uint8_t)(seq >> 8);
    sbuf[2] = kind;
    sbuf[3] = idx;
    sbuf[4] = (uint8_t)(len & 255);
    sbuf[5] = (uint8_t)(len >> 8);
    sbuf[6] = 0;
    sbuf[7] = 0;
    memcpy(sbuf + 8, pl, len);
    if (dst)
        sendto(fd, sbuf, len + 8, 0, (struct sockaddr *)dst,
               sizeof *dst);
    else
        send(fd, sbuf, len + 8, 0);
}
...
int do_send(void) {
    ...
    addr(&ap, PS);
    for (int u = 0; u < NP; u++)
        txmsg(ss, SEQ, 0, (uint8_t)u, bku[u], PL, &ap);
    txmsg(ss, SEQ, 1, 0, meta, META, &ap);
    int sf = socket(AF_INET, SOCK_DGRAM, 0);
    struct sockaddr_in af;
    addr(&af, PF);
    bind(sf, (struct sockaddr *)&af, sizeof af);
    int served = 0;
    for (;;) {
        if (!waitfd(sf, 1000) || served >= 8)
            break;
        struct sockaddr_in peer;
        socklen_t al = sizeof peer;
        ssize_t n = recvfrom(sf, rbuf, sizeof rbuf, 0,
                             (struct sockaddr *)&peer, &al);
        if (n < 8 || rbuf[2] != 4)
            continue;
        /* reply P+Q back to the requester (hand-rolled: sf is
         * bound, not connected, so plain send() cannot be used) */
        sbuf[0] = SEQ & 255;
        sbuf[1] = SEQ >> 8;
        sbuf[2] = 2;
        sbuf[3] = 0;
        ...
        sbuf[5] = PL >> 8;
        sbuf[6] = sbuf[7] = 0;
        memcpy(sbuf + 8, parP, PL);
        sendto(sf, sbuf, PL + 8, 0, (struct sockaddr *)&peer, al);
        sbuf[2] = 3;
        memcpy(sbuf + 8, parQ, PL);
        sendto(sf, sbuf, PL + 8, 0, (struct sockaddr *)&peer, al);
        served++;
    }
...
int do_recv(void) {
    /* bku rebuilt locally as the known-answer scoring oracle; the
     * repair path (syndromes from meta, parities from fetch) never
     * reads it. */
    build();
    int sr = socket(AF_INET, SOCK_DGRAM, 0);
    struct sockaddr_in ar;
    addr(&ar, PR);
    bind(sr, (struct sockaddr *)&ar, sizeof ar);
    struct timeval tvr = { 0, 500000 };
    ...
    while (got < 9 && it++ < 40) {
        uint8_t k, ix;
        uint16_t ln;
        ssize_t n = recv(sr, rbuf, sizeof rbuf, 0);
        if (n < 8)
            break;
        k = rbuf[2];
        ix = rbuf[3];
        ln = (uint16_t)(rbuf[4] | (rbuf[5] << 8));
    ...
    if (!full && havem && nl >= 1 && nl <= 2) {
        ...
        struct sockaddr_in af;
        addr(&af, PF);
        connect(sf, (struct sockaddr *)&af, sizeof af);
        setsockopt(sf, SOL_SOCKET, SO_RCVTIMEO, &tvr, sizeof tvr);
        uint8_t rq[1] = { 0 };
        txmsg(sf, SEQ, 4, 0, rq, 1, NULL);
        uint8_t fP[PL], fQ[PL];
        int gotP = 0, gotQ = 0;
        for (int m = 0; m < 2; m++) {
            uint8_t fk, fi;
            uint16_t fl;
            ssize_t n = recv(sf, rbuf, sizeof rbuf, 0);
            if (n < 8)
                break;
            fk = rbuf[2];
            fi = rbuf[3];
            fl = (uint16_t)(rbuf[4] | (rbuf[5] << 8));
            (void)fi;
            if (fk == 2 && fl == PL) {
                memcpy(fP, rbuf + 8, PL);
                gotP = 1;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        printf("usage: udp_cpeer send|recv\n");
        return 1;
    }
