...
def gen_init_chain(nres, seq, hydro_values, ang_t0, tors_p0, tors_k=0.6000,
    ...
    lines.append("")
    # phosphate charge flags (1.0 = nucleotide backbone bead)
    if phos_values is not None:
        lines.append("  ! Phosphate charge flags (screened repulsion term)")
        for i, (s, p) in enumerate(zip(seq, phos_values), start=1):
            lines.append(f"  RES_PHOS({i}) := {p:.1f}   ! {s}")
        lines.append("")
...
def gen_init_native(nres, scaled_coords, contacts, scale, cutoff_model, register_entries=None, register_k=0.5,
                    domains=None, cystine=None, mispair=None, chain_break=None, dock_stagger=None):
    ...
    # Contact-layer list: for each native contact pair (i,j), the native
    # vector i->j expressed in i's local Frenet frame (same R^T convention
    # as bondvecs). Inert data; only read when FRAME_CL_K > 0.
    if contacts:
        if len(contacts) > 1024:
            raise ValueError(f"CL list {len(contacts)} exceeds MAXCL 1024 — widen template CL arrays")
        lines.append("")
        lines.append("  ! Contact-layer pairs: native contact vector in i's local frame")
        for p_, (i_, j_, d_) in enumerate(contacts, start=1):
            q_ = quats[i_-1]
            v_ = _quat_rot([q_[0], -q_[1], -q_[2], -q_[3]],
                           np.array(scaled_coords[j_-1]) - np.array(scaled_coords[i_-1]))
            lines.append(f"  CL_I({p_}) := {i_}")
            lines.append(f"  CL_J({p_}) := {j_}")
            lines.append(f"  CL_VX({p_}) := {v_[0]:.6f}")
            lines.append(f"  CL_VY({p_}) := {v_[1]:.6f}")
            lines.append(f"  CL_VZ({p_}) := {v_[2]:.6f}")
    lines.append("")
        ...
        for i, j, d in contacts:
            ...
            if domains is not None:
                ...
                if di is not None and dj is not None and di != dj:
                    if dock_stagger is not None:
                        # pair opens when the later domain "arrives": seam max(di,dj)-1
                        gate = dock_stagger[max(di, dj) - 2]
                        lines.append(f"  CONTACT_INTER({i}, {j}) := {gate}")
                        lines.append(f"  CONTACT_INTER({j}, {i}) := {gate}")
                    else:
...
def patch_template(template_path, nres, init_chain, init_native, cutoff_model, name,
                   ...
                   vmax=None, thermal_floor=None, phos_eps=None, phos_lam=None,
                   frame_cl_k=None, ncl=None):
    ...
    src = re.sub(r'RES_(X|Y|Z|VX|VY|VZ|THETA|OMEGA|ANGK|ANGT0|TORSK|TORSP0|HYDRO|PHOS|Q0|Q1|Q2|Q3|WX|WY|WZ)\(\d+\)',
                 repl_size, src)
    ...
    if phos_eps is not None:
        src = re.sub(r'PARAMETER REAL :: PHOS_EPS = [0-9.eE\-]+',
                     f'PARAMETER REAL :: PHOS_EPS = {phos_eps:.4f}', src)
    if phos_lam is not None:
        src = re.sub(r'PARAMETER REAL :: PHOS_LAM = [0-9.eE\-]+',
                     f'PARAMETER REAL :: PHOS_LAM = {phos_lam:.4f}', src)
    if frame_cl_k is not None:
        src = re.sub(r'PARAMETER REAL :: FRAME_CL_K = [0-9.eE\-]+',
                     f'PARAMETER REAL :: FRAME_CL_K = {frame_cl_k}', src)
    if ncl is not None:
        src = re.sub(r'PARAMETER INTEGER :: NCL = \d+',
                     f'PARAMETER INTEGER :: NCL = {ncl}', src)
...
def generate_variant(name, json_path, pdb_path, chain, output_path,
                     ...
                     domains=None, dock_from=None, dock_stagger=None, scaled_schedule=False,
                     ...
                     vmax=None, thermal_floor=None, phos_eps=None, phos_lam=None,
                     keep_inter_contacts=False, frame_cl_k=None):
    ...
    # Phosphate flags: 1.0 for nucleotide backbone beads (from JSON res_names),
    # 0.0 for amino acids. res_names entry of 'UNK' (no CA in PDB = RNA) → 1.0.
    _AA3 = {'ALA','CYS','ASP','GLU','PHE','GLY','HIS','ILE','LYS','LEU',
            'MET','ASN','PRO','GLN','ARG','SER','THR','VAL','TRP','TYR'}
    # Resolution order per residue: JSON res_names, then PDB seq (seq3).
    # A residue is charged only if its resolved name is a non-AA, non-UNK
    # token (i.e. a nucleotide code like A/U/G/C/2MG/H2U). UNK → 0.0 (safe:
    # protein chains in mixed JSONs often carry UNK res_names).
    def _is_phos(rn, s3):
        name = rn.strip().upper() if rn else 'UNK'
        if name in ('UNK', ''):
            name = s3.strip().upper() if s3 else 'UNK'
        return 0.0 if (name in _AA3 or name in ('UNK', '')) else 1.0
    if res_names is not None:
        phos_values = [_is_phos(rn, s3) for rn, s3 in zip(res_names, seq3)]
    else:
        phos_values = [_is_phos(None, s3) for s3 in seq3]
    ...
    for cb in breaks:
        ...
        if not keep_inter_contacts:
            # MIDY default: chains fully independent (no native inter-chain Go)
    ...
    if dock_stagger is not None:
        if domain_intervals is None:
            raise ValueError("--dock-stagger requires --domains")
        if len(dock_stagger) != len(domain_intervals) - 1:
            raise ValueError(f"--dock-stagger needs {len(domain_intervals)-1} seam frames, got {len(dock_stagger)}")
        if dock_from is not None:
            raise ValueError("--dock-stagger and --dock-from are mutually exclusive")
        # DOCK_FROM left at template default (huge): per-seam CONTACT_INTER gates govern
    if domain_intervals is not None and dock_from is None and dock_stagger is None:
    ...
    init_chain = gen_init_chain(nres, seq1, hydro_values, ang_t0, tors_p0, tors_k=tors_k if tors_k is not None else 0.6000,
                                ...
                                domain_anchors=domain_anchors, phos_values=phos_values)
    ...
    src = patch_template(template_path, nres, init_chain, init_native, cutoff_model, name,
                         ...
                         phos_eps=phos_eps, phos_lam=phos_lam,
                         frame_cl_k=frame_cl_k, ncl=len(contacts))
...
def main():
    ...
    parser.add_argument('--dock-stagger', default=None,
                        help="Per-seam gate frames N->C, e.g. '24000,48000,72000,96000' for 5 domains. "
                             "Pair (i,j) gates at FRAME > stagger[max(dom)-2]. Mutually exclusive with --dock-from.")
    ...
    parser.add_argument('--keep-inter-contacts', action='store_true',
                        help='Assembly mode: keep native Go contacts across chain breaks (default: strip them, MIDY behavior)')
    parser.add_argument('--frame-cl-k', type=float, default=None,
                        help='Contact-layer orientation coupling (second shell). Default/absent: 0.0 = off')
    ...
    parser.add_argument('--phos-eps', type=float, default=None,
                        help='Screened phosphate repulsion strength (default/absent: template 0.0 = off)')
    parser.add_argument('--phos-lam', type=float, default=None,
                        help='Screening length for phosphate repulsion, model units (default: template 3.1)')
    ...
    generate_variant(args.name, args.json, args.pdb, args.chain, args.output,
                     ...
                     dock_stagger=([int(x) for x in args.dock_stagger.split(',')] if args.dock_stagger else None),
                     ...
                     phos_eps=args.phos_eps, phos_lam=args.phos_lam,
                     keep_inter_contacts=args.keep_inter_contacts,
                     frame_cl_k=args.frame_cl_k)
