...
smokes; smokes before ensembles; checkpoint to `/mnt`.

Workflow amendment, 2026-09-02: the user authorized swarm delegation for
implementation, gates, and smoke tests where parallelizable. Stage ordering
remains lock-step; no smoke or ensemble may start before its prerequisite
gate passes.
...
| `XSEED` | 6.0 | dynamic initial seed center; default is exactly parent `LBOX/2` |
| `PADHF` | 0/1 | WRITE-only per-step adhesion-force record; default 0 is inert |
...
measurable at the brush's certified per-tip force scale. `XSEED` is a
default-inert initial-condition control added for the R6-b unit smoke; its
default must leave PADH=0 O-A1 output bit-identical, and only the nondefault
unit smoke may place the seed near the substrate.
...
### Per-step force records

For the R6-d law grid, `PADHF=1` emits one WRITE-only record per step for
each attached bond:

```text
adhf STEP F M force_x force_y force_z
```

These records provide the exact force path required for slip-hazard
estimation and independent reconstruction of every `adhs` traction window.
They draw no RNG, change no state, and are never emitted when PADH=0. The
default `PADHF=0` leaves all existing dynamic and MIRROR outputs unchanged.
...
Event-resolved rupture hazard must increase with adhesion force. With the
PADHF=1 law-grid records, expand each bond into per-step exposure rows using
`F_A(t)=|f_adhf(t)|`. Slip ruptures (cause 1) are events; hard stretch,
monomer unbind, filament death, and bonds open at FINAL are right-censored at
their last attached step. Pooling the law-grid arms, fit the Bernoulli/Poisson
hazard
...
by maximum likelihood. The fitted sensitivity β_A must lie in the registered
window:
...
1. **Bookkeeping exactness:** reported substrate traction equals the
   ...
   tolerance on every `adhs` record. For PADHF=1 law-grid logs, recompute
   each window as `(1/NDIAG)Σ_stepsΣ_bonds(-f_adhf)` and compare against
   `(trx,try,trz)`.
...
The registered default-inert initial-condition parameter is `XSEED=6.0`,
exactly equal to the parent dynamic seed center `LBOX/2`. The unit smoke may
use `XSEED=3.5`, placing the initial pointed tail bead at x≈2.65 inside the
capture slab. Any nondefault value must be documented and must not affect the
PADH=0 O-A1 gate.
...
A preliminary old-schema grid was started after the smokes but cancelled when
the O-A3/O-A4 statistical audit showed that rupture-step force alone cannot
identify β_A and that `adhs` traction cannot be independently recomputed
without per-step forces. Those logs are preserved under
`runs20/law_grid/superseded_*` and are not certification inputs. The
registered grid below must run with PADHF=1.
