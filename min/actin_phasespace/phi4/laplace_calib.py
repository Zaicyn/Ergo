...
def build(snap):
    ...
    for F, d in ftd.items():
        ...
        sinkP += (TO.ball_mask(d['pnt'], RCAP) & ~body).astype(float)
...
def solve_split(pre, betaP, x0=None):
    body, sinkB, sinkP, srcv, Lf = pre
    beta = (1.0*sinkB + betaP*sinkP)*(~body)
    fluid = ~body.ravel()
    ...
    JB = float((sinkB*c).sum()*TO.dV); JP = float(betaP*(sinkP*c).sum()*TO.dV)
